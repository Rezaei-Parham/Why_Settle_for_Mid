...
from loss import compute_loss, compute_loss_multi
...
class Pipeline(
    ...
    def __call__(
        ...
        loss_orders: List[Dict[str, Any]]= None,
                ...
                if not run_standard_sd and i < max_iter_to_alter:
                    with torch.enable_grad():
                        for order in loss_orders:
                            ...
                            latents_2 = self.scheduler.scale_model_input(latents, t)
                            ...
                            attention_list = torch.empty(0, device='cuda')
                            for index in order['indices_to_alter']:
                                att = attention_maps[:, :, index]
                                if True:
                                    smoothing = GaussianSmoothing(channels=1, kernel_size=kernel_size, sigma=sigma, dim=2).cuda()
                                    input = F.pad(att.unsqueeze(0).unsqueeze(0), (1, 1, 1, 1), mode='reflect')
                                    att = smoothing(input).squeeze(0).squeeze(0)
                                attention_list = torch.cat((attention_list, att.unsqueeze(0)), dim=0)

                            loss = 0

                            if  i < max_iter_to_alter:
                                loss = compute_loss(attention_maps=attention_maps,
                                                    indices_to_alter=order['indices_to_alter'],
